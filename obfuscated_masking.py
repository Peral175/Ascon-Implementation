...
class ObfuscatedTransformer(CircuitTransformer):
    ...
    def __init__(self, prng=None, n=2, c=(None, None, None)):
        if c is None:
            c = [None]
        self.prng = prng
        self.n = int(n)
    ...
    def visit_XOR(self, node, x, y):
        ...
        n = self.n
        if c < 50:  # 50% chance for unobfuscated
            # original
            return x ^ y
        elif c < 75:  # 25% chance for simple obfuscated
            # v1
            nodes = self.target_circuit.nodes[-n:]
            o = nodes[random.randint(0, n - 1)]
            return ((o ^ x) ^ y) ^ o
        elif c < 90:  # 15% chance for medium obfuscated
            # v2
            nodes = self.target_circuit.nodes[-n:]
            o1 = nodes[random.randint(0, n - 1)]
            o2 = nodes[random.randint(0, n - 1)]
            return (((o1 ^ x) ^ (o2 ^ y)) ^ o1) ^ o2
        elif c < 100:  # 10% chance for complex obfuscated
            # v3
            nodes = self.target_circuit.nodes[-n:]
            o = nodes[random.randint(0, n - 1)]
    ...
    def visit_AND(self, node, x, y):
        ...
        if c < 50:  # 50% chance for unobfuscated
            # original
            return x & y
        elif c < 80:  # 30% chance for medium obfuscated
            # v1
            nodes = self.target_circuit.nodes[-n:]
            o = nodes[random.randint(0, n - 1)]
            return ((x ^ o) & y) ^ (o | y) ^ o ^ y
        elif c < 100:  # 20% chance for complex obfuscated
            # v2
            nodes = self.target_circuit.nodes[-n:]
            o1 = nodes[random.randint(0, n - 1)]
            o2 = nodes[random.randint(0, n - 1)]
